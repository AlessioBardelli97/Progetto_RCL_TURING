...
/**
 * Classe che implementa lo scambio vero e proprio delle richieste e
 * delle risposte tra il client e il server di TURING mediante protocollo TCP.
 */
...
class WriteAndRead {
    ...
    static void write(Request request, SocketChannel channel) throws IOException {
        ...
        byte[] req = request.getRequest().getBytes();
    ...
    private static void writeDocument(Request request, SocketChannel channel) throws IOException {
        ...
        Request newRequest = new Request(request.getRequest() + "//__//" + new String(buffer.flip().array()));
